...
import { MiseEnPageAdmin } from '@/components/admin/MiseEnPageAdmin'
...
export function PersonnelPage() {
  const { utilisateur, personnel: initial } = useLoaderData<typeof chargerPersonnel>()
  ...
  const modifier = async (id: number, modification: { actif?: boolean; role?: Role }) => {
    ...
    try {
      ...
      if (probleme instanceof ErreurApi && probleme.status === 401) {
        void navigate(`/connexion?retour=${encodeURIComponent('/admin')}`, { replace: true })
      } else {
  ...
  return (
    <MiseEnPageAdmin utilisateur={utilisateur} titre="Personnel">
