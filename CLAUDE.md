...
**Étapes 1 à 9 du §13 terminées** : `backend/` contient Prisma (schéma, migration initiale, seed de la carte et du compte admin), le serveur Express, l'authentification du personnel, la gestion de la carte (catégories, plats, photos Cloudinary), la création de commande par le client, l'API de l'écran cuisine et le temps réel Socket.io. `frontend/` contient la page menu publique `/menu/:jeton`, le panier, la validation (`/menu/:jeton/commande`), le suivi en direct (`/menu/:jeton/commandes/:id`), la connexion du personnel (`/connexion`), l'écran cuisine (`/cuisine`), le back-office (tableau de bord `/admin`, `/admin/commandes`, `/admin/tables`, `/admin/plats`, `/admin/categories`, `/admin/personnel`) et l'espace serveur (`/serveur`, `/serveur/commande`). Écrans du personnel utilisables sans réseau et installables ; menu client gardé sur le téléphone. Prochaine étape : abstraction du paiement (§13, étape 10). Au lancement, `/` redirige toujours vers `/connexion`. Aucun test automatisé : l'API se vérifie avec `backend/requests/requests.rest`.
...
- **Budget §8, mesuré dans le navigateur** (fichiers réellement chargés par `/menu/:jeton`, gzip niveau 9) : 140,9 Ko de JavaScript (mesure du 2026-09-15, après l'étape 9), dont 92,4 Ko pour l'entrée (surtout React et React Router), environ 30 Ko de morceaux partagés (Radix, utilitaires) et 7,1 Ko pour la page. Le client Socket.io (12,9 Ko) n'est chargé que par le suivi et l'écran cuisine, jamais par le menu. Remesurer après chaque ajout de dépendance côté client : Vite redécoupe les morceaux partagés, la taille d'un seul fichier ne suffit pas.
  ...
  - **Photos toujours affichées par défaut** (décision du 2026-09-16, qui l'emporte sur le mode texte automatique du §8) : elles vendent les plats, y compris sur un mauvais réseau. Le repli texte reste disponible, mais au choix du client : un bouton dans l'en-tête de la carte coupe ou remet les photos, y compris les vignettes des catégories, et le choix est gardé sur l'appareil (`stores/preferences.ts`, `photos: 'oui' | 'non'`).
  - Détection automatique retirée (`lib/reseau.ts` supprimé) : `effectiveType` mesure le débit réel et non le type de réseau, donc Chrome annonce « 2g » sur une 4G chargée. Résultat constaté en ligne sur MTN Douala le 2026-09-16 : aucune photo, sans moyen de les afficher. Ce n'est qu'un réglage par défaut : un bouton dans l'en-tête de la carte affiche ou masque les photos, et le choix est gardé sur l'appareil (`stores/preferences.ts`, `photos: 'auto' | 'oui' | 'non'`). Quand elles sont masquées automatiquement, un bandeau « Photos masquées pour économiser votre connexion » propose de les afficher. Sans ce bouton, un client en économiseur de données ne voyait jamais les photos (constaté en ligne le 2026-09-16).
...
- **Temps réel** (`src/sockets/io.ts`, Socket.io 4.8.3) :
  ...
  - Salles §9 : ADMIN et CUISINE rejoignent `:cuisine`, ADMIN et BAR rejoignent `:bar`, ADMIN et SERVEUR rejoignent `:serveur`, un client rejoint la salle de sa table. Une commande est diffusée aux trois salles du personnel ; chaque écran ne garde que son poste.
  - Appels des tables : `diffuserAppel` envoie `table:appel-serveur` ou `table:addition` (appel complet, montant compris) à `:serveur`, `diffuserAppelTraite` envoie `appel:traite` `{ id }`.
...
- **Rôles du personnel** : ADMIN, CUISINE, BAR, SERVEUR (décision du 2026-09-16, §10). **CUISINE ne voit que les plats, BAR que les boissons**, d'après le poste de la ligne, hérité de la catégorie. L'admin voit les deux files. `postesDuRole(role)` (`cuisine.service.ts`) donne les postes visibles ; le filtrage et l'autorisation sont **côté serveur** : toucher l'article d'un autre poste renvoie 403, l'écran ne fait pas foi. Cuisine et bar partagent l'écran `/cuisine`, qui n'affiche que la file du rôle connecté. L'inscription propose Cuisine, Bar ou Serveur ; l'admin peut changer le rôle depuis `/admin/personnel`.
- **Écran cuisine et bar** (`/api/cuisine/*`, rôles CUISINE, BAR et ADMIN ; `frontend/src/routes/cuisine/`) :
...
- **Espace serveur** (`/api/serveur/*`, rôles SERVEUR et ADMIN ; `src/services/salle.service.ts`, `frontend/src/routes/serveur/`) :
  - `GET /salle` : appels en attente, et par table les lignes à servir (PRETE), en cours (RECUE, EN_PREPARATION) et servies, total non encaissé hors annulations, sur 12 h. Tables triées : appels, puis plats prêts, puis numéro.
  - `PATCH /lignes/servies` `{ ligneIds }` : PRETE → SERVIE, même principe que la cuisine (updateMany conditionnel, 409 si une ligne a changé). Seul le serveur marque « servie » (§6).
  - `POST /lignes/:id/annulation` `{ motif }` : uniquement depuis RECUE ; ensuite 409, seule la cuisine peut annuler (§6).
  - `POST /tables/:id/encaissement` `{ modePaiement }` : pose `encaisseeAt`, `encaisseeParId` et le mode sur toutes les commandes non encaissées de la table (12 h), dans une transaction ; ferme les demandes d'addition de la table. Rien à encaisser : 409. À valider uniquement après réception de l'argent ; des plats pas encore servis sont signalés sans bloquer.
  - `PATCH /appels/:id/traite` (« J'y vais ») ; `GET /menu` : carte et tables actives pour la saisie.
  - `POST /commandes` `{ tableId, chaise (facultative), cleIdempotence, lignes }` : même service que la commande client (`enregistrerCommande`), avec `source: SERVEUR` et `saisieParId`. Aucune limite de débit.
  - Appels du client : `POST /api/menu/:jeton/appels` `{ type: 'APPEL_SERVEUR' }` ou `{ type: 'ADDITION', modePaiement }`. Une demande en attente du même type est mise à jour, pas dupliquée. Addition sans montant : 409. Limite : 6 appels par table sur 10 minutes (compte aussi les requêtes invalides).
  - Écran `/serveur` (`useSalle`) : chaque événement relit `GET /salle` (regroupé sur 300 ms), carillon quand un plat devient prêt ou qu'une table appelle. Bandeaux d'appels, onglets « À servir » et « Tables » (détail, ajout, encaissement, annulation avant préparation).
  - Saisie `/serveur/commande` (`SaisiePage`) : table, place facultative, recherche et catégories, « + » ajoute directement un article sans choix, sinon la fiche du client (`FichePlat` avec `onAjouter`). Panier local à la page (pas le store du client) ; clé d'idempotence fixe tant que la page est ouverte.
  - Côté client, le suivi affiche « Le serveur » (`ActionsTable` : appeler, payer en espèces, payer par Mobile Money) tant que la commande n'est pas encaissée, puis « Commande payée » ; il reste connecté au temps réel jusqu'à l'encaissement.
  - Encaissement visible en base par `Commande.encaisseeAt` ; le format public expose `encaissee`.
- **Paiement** (étape 10, §4 ; `src/services/paiement.service.ts`) :
  - **Moyens** : `ModePaiement` = `ESPECES`, `ORANGE_MONEY`, `MTN_MOMO`, `CARTE`. `MOBILE_MONEY` reste dans l'enum de la base pour les anciennes commandes et pour ne pas casser le site en ligne pendant un déploiement, mais n'est plus accepté en entrée. La carte est prévue par l'abstraction et n'est pas proposée au client tant qu'aucun prestataire n'est branché.
  - **Un seul prestataire aujourd'hui** : `paiementConstateParLeServeur`. Il ne contacte aucune passerelle : il donne le numéro marchand à créditer, et c'est le serveur qui confirme la réception (§6). Brancher MTN MoMo, Orange Money ou un agrégateur plus tard revient à ajouter une implémentation de `Prestataire` et à changer l'export `prestataire`.
  - **Numéros marchands** : `Restaurant.numeroOrangeMoney` et `numeroMtnMomo`, réglés dans `/admin/reglages` (`GET /api/gestion/restaurant`, `PATCH /api/gestion/restaurant/paiement`). Validation : 9 chiffres commençant par 6, indicatif `+237` accepté puis retiré, chaîne vide = numéro retiré. Un moyen sans numéro renvoie 409 au client.
  - **Client** : après « Payer par Orange Money » ou « MTN MoMo », le suivi affiche le numéro à créditer et le montant (`AppelSalle.numeroPaiement`). **Serveur** : le même numéro apparaît dans le dialogue d'encaissement (`GET /serveur/salle` renvoie `restaurant`), à lire au client.
  - **Compte marchand** : le vrai paiement automatique reste bloqué tant que le restaurant n'a pas son compte marchand et son dossier (CNI, NIU, RCCM). API directes (MTN MoMo, Orange Money Web Payment) ou agrégateur local : une seule intégration pour Orange, MTN et carte, 1,5 à 3 % par transaction, 1 à 2 semaines de mise en service.
- **Résilience** (étape 9, §7) :
  - **Cache local** `lib/cacheLocal.ts` : IndexedDB natif, sans dépendance. Toute erreur (navigation privée, stockage plein) est journalisée et vaut un cache vide.
  - **Menu client** (`routes/client/menu.loader.ts`) : un seul menu gardé, celui de la dernière table. Même jeton : affichage immédiat depuis le cache, même sans réseau, puis téléchargement en arrière-plan. Si la carte a changé, le cache est remplacé, le jeton est ajouté à `menusRafraichis` (`lib/menuFrais.ts`, lu par `shouldRevalidate` de la route `menu`) et `router.revalidate()` réaffiche. Jeton refusé (400 ou 404) : cache effacé, la relecture affiche l'erreur. Les prix affichés restent indicatifs : le serveur recalcule tout.
  - **Écrans du personnel** : `chargerEcranPersonnel(request, clé, rôles, lire)` (`lib/ecranPersonnel.ts`) dans les loaders de `/cuisine` (clé `cuisine`), `/serveur` (`salle`) et `/serveur/commande` (`menu-serveur`). En ligne : session vérifiée, données lues puis enregistrées. Erreur réseau (status 0) : dernière version enregistrée, si son rôle est autorisé ; une session expirée (401) ou un refus (403) ne passent jamais par le cache. `useMemoireEcran` réenregistre chaque version reçue du serveur (appeler `recu()` avant de l'appliquer). `BandeauHorsLigne` affiche l'heure des dernières données tant que le socket est coupé.
  - Hors ligne, les actions ne sont **pas mises en file** : elles échouent avec un message, pour ne jamais rejouer un changement de statut périmé.
  - **Envoi de commande du client sans réseau** (`ValidationPage`) : message « Pas de connexion… appelez le serveur », panier et clé d'idempotence conservés, bouton « Réessayer maintenant », et renvoi automatique à l'événement `online` tant que la page reste ouverte. Rien ne part après le départ du client : aucune file d'attente en arrière-plan. La saisie serveur garde son panier et sa clé d'idempotence : « Envoyer » au retour du réseau ne crée pas de doublon.
  - `deconnecter()` efface les clés `CLES_PERSONNEL` : un autre compte ne voit jamais ces données sur l'appareil.
  - **Application installable du personnel** (`vite-plugin-pwa` 1.3.0, `generateSW`) : `injectRegister: false` et `manifest: false`. `useApplicationPersonnel` (appelé par `EnteteStaff`) lie `public/manifest.webmanifest` et enregistre le service worker : le téléphone d'un client n'en a jamais (précache d'environ 800 Ko). `registerType: 'prompt'` : une nouvelle version déployée affiche « Mettre à jour » dans l'en-tête, jamais de rechargement en plein service (le son serait coupé). Service worker absent en `npm run dev` : tester avec `npm run build:ui` puis le port 3001.
  - Icônes `public/icones/` générées depuis le logo basse résolution de `design/brand/` : à refaire avec le logo HD du restaurant.
  - **Cache HTTP** (`backend/src/index.ts`) : `/assets/*` (noms hashés) `public, max-age=31536000, immutable` ; tout le reste (page, `sw.js`, manifest, icônes) `no-cache`.
- **Navigation du personnel** (décision du 2026-09-15 : elle doit rester évidente) :
  ...
  - Menu du back-office unique (`components/admin/NavigationAdmin.tsx`) : colonne fixe sur ordinateur, panneau ouvert par ☰ sur téléphone (refermé après un choix). Sections : Aujourd'hui, Restaurant, Carte, Équipe, Service. Toute nouvelle page s'y ajoute.
...
- **Tables et QR codes** (`/api/tables`, ADMIN ; `routes/admin/TablesPage.tsx`, `TablePage.tsx`, `QrTablesPage.tsx`) :
  ...
  - Suppression douce (`actif: false`, jeton changé) : l'ancien QR cesse de fonctionner, les commandes passées gardent leur table. Refusée si la table a des commandes non encaissées **de moins de 12 h**. Recréer un numéro supprimé réactive la même table avec un nouveau jeton.
  ...
  - **Adresse encodée dans le QR** : `URL_PUBLIQUE` (backend/.env) en production, sinon l'adresse par laquelle l'admin ouvre le site. En développement, `localhost` est remplacé par l'IP de l'ordinateur sur le réseau local (`src/lib/adresse.ts`), pour qu'un téléphone sur le même Wi-Fi puisse scanner. Il faut aussi que le pare-feu laisse passer le port 3001 : `sudo ufw allow from 192.168.88.0/24 to any port 3001 proto tcp`. Si c'est une adresse locale (localhost, 192.168…), la page d'impression l'affiche en avertissement. En production, définir `URL_PUBLIQUE` avant d'imprimer les fiches.
...
- **Carte dans le back-office** (API de l'étape 3, écrans `routes/admin/PlatsPage.tsx`, `PlatPage.tsx`, `CategoriesPage.tsx`) :
  - Catégories : poste Cuisine ou Bar, ordre des onglets de la carte (flèches qui échangent deux positions), archivage refusé si la catégorie contient des plats.
  - Plats : recherche, filtre par catégorie (`?categorie=`) et par disponibilité, interrupteur « disponible » dans la liste. Fiche à onglets (`?onglet=`) : Informations, Photo, Tailles et choix, Extras, Compléments ; `/admin/plats/nouveau` crée le plat puis ouvre l'onglet Photo.
  - Tailles, extras et compléments s'enregistrent en listes complètes (l'API remplace les précédentes) : l'onglet édite un brouillon, puis « Enregistrer » ou « Annuler ».
  - Photos : `envoyerPhoto()` (`lib/cloudinary.ts`) envoie le fichier directement à Cloudinary avec les champs signés, puis l'API vérifie la signature. Une photo d'option (parfum…) passe par le même envoi.
  - Serveur lent (Neon) : changer de filtre ou d'onglet ne relit pas l'API (`shouldRevalidate` : même chemin, seuls les paramètres changent). Après un enregistrement, la fiche reprend le plat renvoyé par l'API au lieu de relire toute la carte.
  - Montants saisis avec `ChampMontant` (chiffres uniquement, entiers en FCFA).
- **Tableau de bord et commandes du jour** (`/api/gestion/*`, ADMIN, lecture seule ; `src/services/gestion.service.ts`) :
  - **Journée à l'heure de Douala** (UTC+1, sans heure d'été) : `bornesJournee(jour)` (`src/lib/journee.ts`) donne minuit à minuit heure locale, car Render tourne en UTC. Le frontend formate aussi dates et heures avec le fuseau `Africa/Douala` (`lib/journee.ts`), quel que soit l'appareil de l'admin.
  - `GET /tableau-de-bord?date=AAAA-MM-JJ` : nombre de commandes et chiffre d'affaires hors annulations, panier moyen, commandes en cours, 5 plats les plus commandés, 5 dernières commandes.
  - **Total encaissé** : somme des commandes dont `encaisseeAt` tombe ce jour-là, même passées la veille. Seul le serveur encaisse.
  - `GET /commandes?date=&statut=&table=` (numéro de table) et `GET /commandes/:id` : le statut d'une commande se déduit de ses lignes (`statutCommande`), il est donc filtré après calcul. Le détail donne, par ligne, les options, l'instruction, les heures de préparation, prête et servie, et le motif d'annulation.
  - `validerQuery` valide `req.query`. Express 5 interdit de remplacer `req.query` : le contrôleur relit la valeur avec le même schéma.
  - Pas de graphique en v1 : tuiles de chiffres façon FoodScan et listes.
- **Tableaux sur téléphone** : toute zone `overflow-x-auto` porte aussi `relative`. Sinon, les éléments `sr-only` d'un tableau (positionnés en absolu) s'y placent par rapport à la page et élargissent tout l'écran. Dans une grille, un élément qui contient du texte sans retour à la ligne doit porter `min-w-0`.
...
**Cache** : fichiers hashés de `assets/` gardés un an (`immutable`), page, service worker et manifest en `no-cache` (voir « Résilience » au §0).
